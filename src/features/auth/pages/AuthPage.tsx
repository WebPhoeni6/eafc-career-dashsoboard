...
export const AuthPage: React.FC = () => {
  const queryMode = useMemo(() => {
    ...
    return raw === 'forgot' || raw === 'reset' || raw === 'signup' || raw === 'login' ? raw : null;
  }, []);
  ...
  const [mode, setMode] = useState<Mode>((queryMode as Mode | null) || (queryToken ? 'reset' : 'login'));
  ...
  const [devResetHint, setDevResetHint] = useState<{ token?: string; url?: string } | null>(null);
      ...
      >
        ...
        {devResetHint?.token && (
          ...
          >
            ...
            <span style={{ wordBreak: 'break-all', color: 'var(--muted)' }}>{devResetHint.token}</span>
            {devResetHint.url && <span style={{ wordBreak: 'break-all', color: 'var(--muted)' }}>{devResetHint.url}</span>}
          </div>
        )}
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '8px' }}>
          {mode !== 'reset' && (
            <button
              ...
              style={{ border: 'none', background: 'none', color: 'var(--muted)', fontSize: '12px' }}
            >
              Have a reset token?
            </button>
          )}
